...
|---|---|---|
| `quick init` | Initialize project (downloads AGENTS.md, creates starter files) | [init.md](cli/init.md) |
| `quick deploy <dir> <subdomain>` | Deploy directory to subdomain | [deploy.md](cli/deploy.md) |
| `quick serve [dir] [sitename]` | Local dev server with full API access | [serve.md](cli/serve.md) |
| `quick auth` | Authenticate for MCP (Google OAuth) | [auth.md](cli/auth.md) |
...
For team projects, put your site in a GitHub repo and use the [CI/CD actions](recipes/ci.md) to auto-deploy on merge. PRs get preview environments that clean up automatically. This is the recommended way to collaborate on a Quick site.
