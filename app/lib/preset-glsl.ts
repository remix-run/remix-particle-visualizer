...
//   uniform sampler2D uModelTex0..4;
//   uniform float     uModelCount0..4;
...
export const PRESET_GLSL = /* glsl */ `
  ...
  // Each preset binds a fixed model slot (Mockups→0, Racecar→1, Runner→2,
  // RacetrackCar→3, Logo→4), so callers pass the matching sampler+count directly
  ...
  /* ── model preset: Remix Logo / Website Mockups ───────── */
    ...
    out vec3 pos, out vec3 col)
  {
    ...
    vec3 mp = sampleModelTex(uModelTex4, uModelCount4, fi);
    float px = mp.x * scale;
    float py = mp.y * scale;
    float pz = mp.z * scale;
